"""
Few-shot ablation: Both examples have NER + dialogue.
Uses v1 fine-tuned model (models/mistral_finetune/best/) with vLLM.
Examples: training IDs 1 and 3747. Output: outputs/submission_fewshot_ner2.csv
"""
...
OUTPUT_CSV = Path("outputs/submission_fewshot_ner2.csv")
...
def build_prompt_ner2(ex1, ex2, eval_row):
    """Both examples have NER + dialogue."""
...
=== Example 2 ===
Extracted Clinical Entities:
{format_entities_for_prompt(ex2["entities"])}
...
def maybe_truncate_and_build(ex1, ex2, row, tok):
    ...
    prompt = build_prompt_ner2(ex1, ex2, eval_row)
    ...
    return build_prompt_ner2(ex1, ex2, eval_row)


def main():
    print("=" * 60)
    print("Few-shot NER2 – Both examples have NER + dialogue")
    print("=" * 60)
