...
# Paper Figure 20 exact settings
params = SamplingParams(temperature=0.5, top_p=1.0, max_tokens=4000)
lora = LoRARequest("mistral_ft", 1, "models/mistral_finetune/best")

print("Generating with paper config: v0.1, temp=0.5, top_p=1.0, max_tokens=4000")
...
for i, out in enumerate(outputs):
    rows.append({
        "id": df.iloc[i]["id"],
        "generated_note": out.outputs[0].text
    })

pd.DataFrame(rows).to_csv("outputs/submission_paper_v01.csv", index=False)
