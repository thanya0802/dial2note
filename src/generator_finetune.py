...
COMBINED_TRAIN_CSV = PROCESSED_DIR / "shared_task_train_combined.csv"
COMBINED_EVAL_CSV = PROCESSED_DIR / "shared_task_eval_combined.csv"
OUTPUT_DIR_MISTRAL = Path("models/mistral_finetune")
OUTPUT_DIR_COMBINED = Path("models/mistral_finetune_combined")
MODEL_MISTRAL_COMBINED = "unsloth/mistral-7b-instruct-v0.1-bnb-4bit"
...
def load_combined_soap_dataset(
    train_path: Path | str = COMBINED_TRAIN_CSV,
    eval_path: Path | str = COMBINED_EVAL_CSV,
) -> tuple[pd.DataFrame, pd.DataFrame | None]:
    """
    Load combined train CSV (required) and eval CSV (optional).
    Columns: id, dialogue, note, entities_json, retrieved_note.
    """
    train_path = Path(train_path)
    if not train_path.exists():
        sys.exit(f"[ERROR] {train_path} not found. Run: python -m src.build_combined_training_data")
    train_df = pd.read_csv(train_path, engine="python", on_bad_lines="warn")
    required = {"id", "note", "dialogue", "entities_json", "retrieved_note"}
    missing = required - set(train_df.columns)
    if missing:
        sys.exit(f"[ERROR] combined train CSV missing columns: {missing}.")

    eval_df: pd.DataFrame | None = None
    eval_path = Path(eval_path)
    if eval_path.exists():
        eval_df = pd.read_csv(eval_path, engine="python", on_bad_lines="warn")
        missing_e = required - set(eval_df.columns)
        if missing_e:
            sys.exit(f"[ERROR] combined eval CSV missing columns: {missing_e}.")
    print(f"Loaded combined train: {len(train_df)} rows from {train_path}")
    if eval_df is not None:
        print(f"Loaded combined eval:  {len(eval_df)} rows from {eval_path}")
    else:
        print(f"[WARN] No {eval_path} — training without a separate eval set.")
    return train_df, eval_df


def finetune_mistral_combined(
    train_df: pd.DataFrame,
    eval_df: pd.DataFrame | None,
    model_name: str = MODEL_MISTRAL_COMBINED,
    output_dir: str | Path = OUTPUT_DIR_COMBINED,
    *,
    load_in_4bit: bool = True,
    lora_r: int = 16,
    lora_alpha: int = 16,
    ...
    """
    Entity-conditioned + RAFT combined fine-tuning (Mistral v0.1 + Unsloth QLoRA).
    Training text matches ``build_combined_training_text`` in ``src.combined_prompt``.
    """
    from src.combined_prompt import build_combined_training_text

    try:
        from unsloth import FastLanguageModel
    except ImportError as e:
        raise ImportError("Mistral+Unsloth requires: pip install unsloth") from e
    ...
    def row_to_text(row: pd.Series) -> str:
        return build_combined_training_text(
            str(row["dialogue"]).strip(),
            str(row["note"]).strip(),
            str(row["retrieved_note"]).strip(),
            row["entities_json"],
        )
    ...
    print(f"\n--- Loading {model_name} (Unsloth 4-bit QLoRA, combined) ---")
    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name=model_name,
        max_seq_length=max_seq_length,
        dtype=None,
    ...
    print(f"\n--- Starting Mistral combined QLoRA fine-tuning ---")
    print(f"  Epochs: {num_train_epochs}")
    print(f"  Batch size: {per_device_train_batch_size} x grad_accum={gradient_accumulation_steps}")
    print(f"  LR: {learning_rate}, scheduler: {lr_scheduler_type}, warmup_steps: {warmup_steps}")
    print(f"  Max seq length: {max_seq_length}")
    print(f"  Output: {output_dir}")
...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(description="Dial2Note – Mistral-7B QLoRA (Unsloth)")
    ap.add_argument(
        "--combined",
        action="store_true",
        help="Entity-conditioned + RAFT combined training (v0.1, shared_task_train_combined.csv).",
    )
    args = ap.parse_args()
    ...
    if args.combined:
        train_df, eval_df = load_combined_soap_dataset()
        finetune_mistral_combined(train_df, eval_df, output_dir=OUTPUT_DIR_COMBINED)
    else:
